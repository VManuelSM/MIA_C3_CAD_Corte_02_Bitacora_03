"""GPU classification strategies for the e-commerce text-mining pipeline (Actividad 3).

Mirrors the design of ``classification_models.py`` (Bitácora 02, CPU): every
algorithm required by the activity (Random Forest, Logistic Regression, SVM)
is encapsulated behind a common ``GPUClassifierStrategy`` interface (*Strategy*
pattern), with a *Template Method* (``run``) that uniformly times the
``fit``/``predict`` cycle, computes accuracy, builds the classification report
and saves the confusion matrix.

The key architectural difference from the CPU version is the absence of
``concurrent.futures.ProcessPoolExecutor``: a single GPU is one device, so
"one algorithm per core" has no GPU analogue. The three strategies here are
run **sequentially** by ``run_gpu_classifiers`` -- this is a deliberate
consequence of the hardware, not an oversight, and is the point the activity
asks us to demonstrate.

Estimators come from RAPIDS cuML (``cuml.ensemble``, ``cuml.linear_model``,
``cuml.svm``), a GPU-accelerated, scikit-learn-compatible API. Metrics
(``classification_report``, ``confusion_matrix``) are computed with
scikit-learn on the host, since predictions are brought back to the CPU
after inference.
"""

from __future__ import annotations

import time
from abc import ABC, abstractmethod
from dataclasses import dataclass
from pathlib import Path

import matplotlib.pyplot as plt
from sklearn.metrics import ConfusionMatrixDisplay, classification_report, confusion_matrix


@dataclass(frozen=True)
class GPUClassificationResult:
    """Outcome of training and evaluating a single classifier on GPU."""

    name: str
    accuracy: float
    train_seconds: float
    predict_seconds: float
    report: str
    matrix_path: Path

    @property
    def total_seconds(self) -> float:
        return self.train_seconds + self.predict_seconds


class GPUClassifierStrategy(ABC):
    """Common interface for every GPU classification strategy."""

    #: Human-readable name of the algorithm (used in filenames and reports).
    name: str
    #: cuML's RandomForestClassifier does not accept sparse input (unlike
    #: LogisticRegression/SVC); subclasses that need it set this to True so
    #: ``run`` densifies the TF-IDF matrix just for that estimator.
    requires_dense: bool = False

    @abstractmethod
    def build(self):
        """Return a fresh, unfitted cuML estimator."""

    def run(self, X_train, y_train, X_test, y_test, target_names, output_dir: Path) -> GPUClassificationResult:
        """Fit the estimator, predict on the test set, time both stages and
        persist the confusion matrix as a PNG under ``output_dir``."""
        estimator = self.build()

        if self.requires_dense:
            X_train = X_train.toarray()
            X_test = X_test.toarray()

        start = time.perf_counter()
        estimator.fit(X_train, y_train)
        train_seconds = time.perf_counter() - start

        start = time.perf_counter()
        y_pred = estimator.predict(X_test)
        predict_seconds = time.perf_counter() - start

        # cuML predictions may come back as cuDF/CuPy objects; bring them to
        # host (NumPy) so scikit-learn's metrics can consume them.
        y_pred_host = y_pred.get() if hasattr(y_pred, "get") else y_pred
        y_test_host = y_test.get() if hasattr(y_test, "get") else y_test

        accuracy = float((y_pred_host == y_test_host).mean())
        report = classification_report(y_test_host, y_pred_host, target_names=target_names)

        output_dir.mkdir(parents=True, exist_ok=True)
        matrix_path = output_dir / f"{self.name.replace(' ', '_')}_GPU.png"
        matrix = confusion_matrix(y_test_host, y_pred_host)
        display = ConfusionMatrixDisplay(confusion_matrix=matrix, display_labels=target_names)
        fig, ax = plt.subplots(figsize=(6, 6))
        display.plot(ax=ax, cmap="Blues", colorbar=False, xticks_rotation=45)
        ax.set_title(f"{self.name} GPU")
        fig.tight_layout()
        fig.savefig(matrix_path, dpi=150)
        plt.close(fig)

        return GPUClassificationResult(
            name=self.name,
            accuracy=accuracy,
            train_seconds=train_seconds,
            predict_seconds=predict_seconds,
            report=report,
            matrix_path=matrix_path,
        )


class RandomForestGPUStrategy(GPUClassifierStrategy):
    """Random Forest classifier (cuML), GPU-accelerated bagging of decision trees."""

    name = "Random Forest"
    requires_dense = True

    def __init__(self, n_estimators: int = 200, random_state: int = 42):
        self.n_estimators = n_estimators
        self.random_state = random_state

    def build(self):
        from cuml.ensemble import RandomForestClassifier

        return RandomForestClassifier(n_estimators=self.n_estimators, random_state=self.random_state)


class LogisticRegressionGPUStrategy(GPUClassifierStrategy):
    """Multinomial logistic regression (cuML), GPU-accelerated linear classifier."""

    name = "Regresion Logistica"

    def __init__(self, max_iter: int = 1000):
        self.max_iter = max_iter

    def build(self):
        from cuml.linear_model import LogisticRegression

        return LogisticRegression(max_iter=self.max_iter)


class SVMGPUStrategy(GPUClassifierStrategy):
    """Support Vector Machine with RBF kernel (cuML), GPU-accelerated."""

    name = "SVM"

    def __init__(self, kernel: str = "rbf"):
        self.kernel = kernel

    def build(self):
        from cuml.svm import SVC

        return SVC(kernel=self.kernel)


def run_gpu_classifiers(strategies, X_train, y_train, X_test, y_test, target_names, output_dir: Path):
    """Run every strategy **sequentially** (one GPU, no process pool) and
    return the list of :class:`GPUClassificationResult`, printing a
    professor-style report block for each algorithm as it finishes."""
    results = []
    for strategy in strategies:
        print("*" * 27)
        print(f"{strategy.name} GPU")
        print("*" * 27)

        result = strategy.run(X_train, y_train, X_test, y_test, target_names, output_dir)
        results.append(result)

        print(f"Accuracy: {result.accuracy:.4f}\n")
        print(result.report)
        print(f"Matriz guardada: {result.matrix_path}")
        print(f"Tiempo: {result.total_seconds:.2f}s\n")

    return results


#: Canonical ordering of the three strategies, matching the activity diagram.
DEFAULT_GPU_STRATEGIES = [
    RandomForestGPUStrategy(),
    LogisticRegressionGPUStrategy(),
    SVMGPUStrategy(),
]
